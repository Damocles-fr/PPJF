// ==UserScript==
// @name         PPJF - PotPlayer launcher (Jellyfin 10.11+ / multi-versions aware)
// @namespace    local.ppjf.potplayer
// @version      10.11.6
// @description  Intercept Play/Resume clicks and open the selected MediaSource path via potplayer://
// @author       Damocles-fr
...
(() => {
  ...
  // ---- Multi versions support (detail page selectSource/detailTrackSelect) ----
  const getSelectedMediaSourceId = () => {
    const candidates = [
      'select.selectSource.detailTrackSelect',
      'select.selectSource',
      'select.detailTrackSelect'
    ];
    ...
    // fallback: any <select> whose selected option value looks like a MediaSource id
  ...
  const resolvePathFromItem = async (itemId, mediaSourceId = null, depth = 0) => {
    ...
    // If a specific MediaSource was selected in the UI, honor it first
    if (mediaSourceId && ms && ms.length) {
      const wanted = ms.find(x =>
        String(x?.Id || '') === mediaSourceId ||
        String(x?.MediaSourceId || '') === mediaSourceId ||
        String(x?.ItemId || '') === mediaSourceId
      );
      if (wanted && wanted.Path) return wanted.Path;
    }

    // Single-source / default behavior
    if (item && item.Path && (!ms || ms.length <= 1)) return item.Path;
  ...
  let _lastLaunchAt = 0;
  const launchPotplayer = async (itemId, mediaSourceId = null) => {
    const now = Date.now();
    ...
    if (!p) {
      console.warn('[PPJF] Unable to resolve a local Path for itemId:', itemId, 'mediaSourceId:', mediaSourceId);
      return;
    }
  ...
  const onUserActivate = (e) => {
    ...
    const mediaSourceId = getSelectedMediaSourceId();

    stopEvent(e);
    launchPotplayer(itemId, mediaSourceId);
  };
